...
def notification_sink():
  ...
  try:
    ...
    etype, efile, eline = utils.extractExceptionData(sys.exc_info())
    log.log(log.LOG_WARNING, "Exception during /docker/token endpoint processing: " + str(etype) + ", in file: " + str(efile) + " at line " + str(eline))
    return response, 500
...
def authenticate():
  ...
  try:
    ...
    #test exception -  for catching and logging unhandled exceptiopns in the endpoint implementation
    #raise IOException()
    ...
    etype, efile, eline = utils.extractExceptionData(sys.exc_info())
    log.log(log.LOG_WARNING, "Exception during /authenticate endpoint processing: " + str(etype) + ", in file: " + str(efile) + " at line " + str(eline))
    return response, 500
...
def getACL():
  ...
  try: 
    ...
    etype, efile, eline = utils.extractExceptionData(sys.exc_info())
    log.log(log.LOG_WARNING, "Exception during /acl(GET) endpoint processing: " + str(etype) + ", in file: " + str(efile) + " at line " + str(eline))
    return response, 500
...
def putACL():
  ...
  try:
    ...
    etype, efile, eline = utils.extractExceptionData(sys.exc_info())
    log.log(log.LOG_WARNING, "Exception during /acl(PUT) endpoint processing: " + str(etype) + ", in file: " + str(efile) + " at line " + str(eline))
    return response, 500
...
def catalog():
  ...
  try:
    ...
    try: 
      ...
      log.log(log.LOG_DEBUG, "Catalog request with filter set to: " + filter)
 
      #expand the fiter (if defined) to a usable regex
      filter = filter.replace(USERNAME, USERNAME_REGEX)
    except KeyError:
      log.log(log.LOG_DEBUG, "Catalog request with empty filter field")
      pass
    ...
    #print "CATALOG CONTENT: " + json.dumps(repo_list)
    log.log(log.LOG_DEBUG, "Catalog content: " + str(json.dumps(repo_list)))
    return response

  except Exception as e:
    etype, efile, eline = utils.extractExceptionData(sys.exc_info())
    log.log(log.LOG_WARNING, "Exception during /catalog endpoint processing: " + str(etype) + ", in file: " + str(efile) + " at line " + str(eline))
    return response, 500
